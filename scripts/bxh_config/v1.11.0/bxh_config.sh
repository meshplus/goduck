...
function generateNodeConfig() {
  ...
  ${BITXHUBBINPATH}/bitxhub key gen --target ${TARGET}/$2 --passwd bitxhub
...
function rewriteNodeConfig() {
  ...
  # order
  order_line=`sed -n '/\[order\]/=' ${TARGET}/$2/bitxhub.toml | head -n 1`
  order_line=`expr $order_line + 1`
  x_replace "$order_line s/type.*= .*/type = \"$CONSENSUSTYPE\"/" ${TARGET}/$2/bitxhub.toml
